$file = 'c:\Users\admin\Desktop\XQX\P1_首页.html'
$content = [System.IO.File]::ReadAllText($file)

# Old buildChar function (lines 742-989)
# We need to find it by content, not line numbers, since we're working with text
$oldFuncStart = $content.IndexOf("function buildChar(){")
Write-Host "Found buildChar at position: $oldFuncStart"

if ($oldFuncStart -lt 0) {
    Write-Error "Could not find function buildChar()"
    exit 1
}

# Find the matching closing brace
# The function starts at oldFuncStart, we need to find the } that closes it
$braceCount = 0
$funcEnd = -1
$searchStart = $oldFuncStart
$inFunction = $false
for ($i = $oldFuncStart; $i -lt $content.Length; $i++) {
    $c = $content[$i]
    if ($c -eq '{') {
        $braceCount++
        $inFunction = $true
...
if ($funcEnd -lt 0) {
    Write-Error "Could not find closing brace of buildChar()"
    exit 1
}

Write-Host "Function ends at position: $funcEnd"

# Extract line numbers for reporting
$beforeFunc = $content.Substring(0, $oldFuncStart)
$funcStartLine = ($beforeFunc -split "`n").Count
$funcContent = $content.Substring($oldFuncStart, $funcEnd - $oldFuncStart + 1)
$funcFull = $content.Substring($oldFuncStart, $funcEnd - $oldFuncStart + 1)
$afterFunc = $content.Substring($funcEnd + 1)

# Count lines in the old function
$oldFuncLineCount = ($funcFull -split "`n").Count
$funcEndLine = $funcStartLine + $oldFuncLineCount - 1
Write-Host "Old function: lines $funcStartLine - $funcEndLine"
...
# Update petCam.position
$newContent = $newContent.Replace('petCam.position.set(0,1.0,8)', 'petCam.position.set(0,1.2,7)')

# Update petControls.target
$newContent = $newContent.Replace('petControls.target.set(0,0.2,0)', 'petControls.target.set(0,1.2,0)')

# Write back
[System.IO.File]::WriteAllText($file, $newContent, [System.Text.Encoding]::UTF8)

# Report new line numbers
$newLines = $newContent -split "`n"
$newFuncStartLine = -1
$newFuncEndLine = -1
for ($i = 0; $i -lt $newLines.Count; $i++) {
    if ($newLines[$i] -match 'function buildChar\(\)\{' -and $newFuncStartLine -eq -1) {
        $newFuncStartLine = $i + 1
    }
    if ($newFuncStartLine -ne -1 -and $newLines[$i].Trim() -eq '}') {
        # Check if next line is blank or comment
        if ($i + 1 -lt $newLines.Count -and ($newLines[$i+1].Trim() -eq '' -or $newLines[$i+1].Trim().StartsWith('//'))) {
            $newFuncEndLine = $i + 1
            break
        }
    }
...
Write-Host "Old buildChar(): lines $funcStartLine - $funcEndLine"
Write-Host "New buildChar(): lines $newFuncStartLine - $newFuncEndLine"

# Verify updates
for ($i = 0; $i -lt $newLines.Count; $i++) {
    if ($newLines[$i] -match 'petCam\.position\.set') {
        Write-Host "petCam.position at line $($i+1): $($newLines[$i].Trim())"
    }
    if ($newLines[$i] -match 'petControls\.target\.set') {
        Write-Host "petControls.target at line $($i+1): $($newLines[$i].Trim())"
    }
}