...
Deno.serve(async (request) => {
  ...
  if (typeof body?.request_id !== "string" || !uuidPattern.test(body.request_id)) return json(request, 400, { ok: false, reason: "invalid_request" });
  try {
    ...
    const rate = await consumeRateLimit(admin, "pet_revive", identity.user.id, request, 10, 600);
    if (!rate.allowed) return json(request, 429, { ok: false, reason: "rate_limited", retry_after_seconds: rate.retryAfterSeconds });
    const { data, error } = await admin.rpc("m3_pet_recover", { p_user_id: identity.user.id, p_request_id: body.request_id });
    if (error || !data?.ok) return json(request, 409, { ok: false, reason: data?.reason || "pet_recover_failed", points_balance: data?.points_balance });
