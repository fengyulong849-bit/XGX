...
Deno.serve(async (request)=>{
  ...
  if(typeof body?.content!=="string"||typeof body?.request_id!=="string"||!uuidPattern.test(body.request_id))return json(request,400,{ok:false,reason:"invalid_content"});
  if(containsSensitive(body.content))return json(request,400,{ok:false,reason:"sensitive_content"});
  try{const admin=serviceClient();const {data:identity,error:identityError}=await admin.auth.getUser(token);if(identityError||!identity.user)return json(request,401,{ok:false,reason:"invalid_session"});const {data,error}=await admin.rpc("m4_create_rant",{p_user_id:identity.user.id,p_content:body.content,p_request_id:body.request_id});if(error||!data?.ok)return json(request,409,{ok:false,reason:data?.reason||"rant_create_failed"});return json(request,200,data);}catch{return json(request,503,{ok:false,reason:"service_unavailable"});}
});
