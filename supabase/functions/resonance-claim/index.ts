...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rate = await consumeRateLimit(admin, "resonance_claim", identity.user.id, request, 30, 600);
    if (!rate.allowed) return json(request, 429, { ok: false, reason: "rate_limited", retry_after_seconds: rate.retryAfterSeconds });
    const { data, error } = await admin.rpc("m4_claim_resonance", { p_user_id: identity.user.id, p_rant_id: body.rant_id, p_request_id: body.request_id });
    if (error || !data?.ok) return json(request, 409, { ok: false, reason: data?.reason || "resonance_claim_failed" });
