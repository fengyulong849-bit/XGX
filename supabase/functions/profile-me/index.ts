...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: profile, error: profileError }, { data: account, error: accountError }, { data: checkins, error: checkinError }] = await Promise.all([
      admin.from("profiles").select("points_balance,free_diy_available,appearance,pet_status,off_hour,off_min,water_interval,sit_interval,created_at").eq("user_id", identity.user.id).maybeSingle(),
